...
class AudioPlayerApp:
    ...
    def create_widgets(self):
        ...
        self.canvas_placeholder.grid(row=0, column=4)
        ...
        self.zoom_in_button.grid(row=0, column=1, padx=0, pady=0)
        
        self.zoom_out_button = tk.Button(self.root, text="Zoom Out", command=self.zoom_out, state=tk.DISABLED)
        self.zoom_out_button.grid(row=0, column=3, padx=0, pady=0)
        ...
        self.label_display_3.grid(row=1, column=2, padx=0, pady=0)
        ...
        self.start_edit_button.grid(row=2, column=1, padx=0, pady=0)
        ...
        self.start_insert_button.grid(row=4, column=1, padx=0)
        ...
        self.save_insert_button.grid(row=4, column=3, padx=0)

        self.edit_entry = tk.Entry(self.root)
        self.edit_entry.grid(row=5, column=2, padx=0)

        self.label_text = tk.StringVar()
        self.label_display_1 = tk.Label(self.root, textvariable=self.label_text)
        self.label_display_1.grid(row=7, column=2, padx=0, pady=0)

        # RIGHT COLUMN
        self.space = tk.StringVar()
        self.space.set('Quality Check')
        self.label_display_2 = tk.Label(self.root, textvariable=self.space)
        self.label_display_2.grid(row=1, column=4, padx=0, pady=0)
        ...
        self.accept_button.grid(row=2, column=4, padx=0, pady=0)
        ...
        self.review_button.grid(row=3, column=4, padx=10, pady=0)
        ...
        self.off_task_button.grid(row=4, column=4, padx=10)
        ...
        self.add_note_button.grid(row=5, column=4, padx=10)

        self.search_button = tk.Button(self.root, text="Search", command=self.search_data, state=tk.DISABLED)
        self.search_button.grid(row=6, column=4, padx=10)

        self.search_result_label = tk.Label(self.root, text="")
        self.search_result_label.grid(row=7, column=4, padx=10)

        self.search_entry = tk.Entry(self.root)
        self.search_entry.grid(row=8, column=4, padx=10)
    ...
    def init_plot(self):
        ...
        self.canvas_widget.grid(row=0, column=4, padx=10, pady=10)
    ...
    def get_audio_segment(self):
        if AUDIO_ENABLED:
            ...
            if self.current_index == len(self.df) - 1:
                ...
                self.end_time = (self.df.iloc[self.current_index+1, 1] * 1000) + 2000
    ...
    def mark_edit_label(self, label, edit_label_value):
        # Check if there's a value in Column1 before updating quality_check_label
        if pd.notna(self.df.at[self.current_index, 'word']):
            self.df.at[self.current_index, 'editor'] = self.initials
    ...
    def edit_data(self, data):
        ...
        if self.df.loc[self.current_index, 'quality_check_label'] == 'insert':
            edit_label = 'insert'
        self.df.loc[self.current_index] = [data, self.onset_position, self.offset_position, confidence] + [edit_label] + [np.nan] + [self.initials] + [np.nan] * (self.df.shape[1] - 7)
        
    def insert_data(self, data):
        index_to_insert = self.current_index + 1 
        
        # Create a new row with None values
        new_row = [data, self.onset_position, self.offset_position] + [np.nan] + ['insert'] + [np.nan] + [self.initials] + [np.nan] * (self.df.shape[1] - 7)  
